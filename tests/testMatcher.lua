...
assertMatch("%w+(?!,)", "apple, banana", true, 1, 4, "Negative lookahead")
...
-- Recursive Groups
assertMatch("^%((?:[^()]+|(?R))*%)$", "(a(b)c)", nil, nil, nil, "Whole-pattern recursion cannot resume after recursive match")
assertMatch("^%((?:[^()]+|(?R))*%)$", "(a(bc)", nil, nil, nil, "Whole-pattern recursion rejects unbalanced parentheses")
assertMatch("%((?:[^()]+|(?R))*%)", "(a(b)c)", true, 1, 7, "Whole-pattern recursion")
assertMatch("(x(?1)?y|A)", "xxAyy", true, 1, 5, "Numbered recursion through alternation")
assertMatch("(x(?1)?y|A)", "xzx", nil, nil, nil, "Numbered recursion failure: recursive call cannot match")
-- Magic Characters
