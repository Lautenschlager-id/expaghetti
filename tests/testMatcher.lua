...
local function assertPOSITION_CAPTURE(expr, str, captureIndex, expectedPos, desc, flags)
	local hasMatched, _, _, metaData = matcher(expr, str, flags)
	assert(hasMatched, string.format("Test '%s': expected match for expr='%s', str='%s'", desc, expr, str))
	local pos = metaData.POSITION_CAPTUREs[captureIndex]
...
assertPOSITION_CAPTURE("()",   "abc", 1, 1,  "Position 1: at start of string (1-based)")
assertPOSITION_CAPTURE("a()",  "abc", 1, 2,  "Position 1: after first char")
assertPOSITION_CAPTURE("ab()", "abc", 1, 3,  "Position 1: after two chars")
assertPOSITION_CAPTURE("()a()", "abc", 1, 1, "Two positions: first at start")
assertPOSITION_CAPTURE("()a()", "abc", 2, 2, "Two positions: second after 'a'")
assertPOSITION_CAPTURE("(a()b)", "ab", 1, 2, "Position inside a capturing group: index is 1 (first pos capture)")
assertPOSITION_CAPTURE("()abc()", "abc", 1, 1, "Position at start and end: first")
assertPOSITION_CAPTURE("()abc()", "abc", 2, 4, "Position at start and end: second")
...
assertPOSITION_CAPTURE("^ã()$", "ã", 1, 3, "Unicode flag off: position capture uses byte offset")
assertPOSITION_CAPTURE("^ã()$", "ã", 1, 2, "Unicode flag on: position capture uses codepoint offset", "u")
